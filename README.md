...
示例：
- ✓ 产生 hint：`core_attributes.hair_color="金色"` → draft 写"林月的紫色发丝" — 明确不同值
- ✗ 不产生 hint：draft 写"她的发丝飘扬" — 中性，无颜色词
- ✗ 不产生 hint：draft 未提及头发 — 缺席 ≠ 矛盾
- ✗ 不误判：同句"林月的银色发丝和陈阳的黑色头发" — 各角色只认领紧邻自己名字的属性
...
├── ui/
│   └── static/
│       ├── index.html          # 静态 Web UI（由 FastAPI 在 port 8000 提供）
│       ├── app.js              # 前端逻辑
│       └── style.css           # 样式
│
└── tests/
